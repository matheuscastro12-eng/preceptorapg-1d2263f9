...
const EnamedBankSimulation = ({ questions, onFinish, onExit, loading }: EnamedBankSimulationProps) => {
  // Chave estável pela lista de IDs das questões do banco (mesma seleção = mesma chave)
  const storageKey = useMemo(
    () => examStorageKey('bank_' + questions.map((q) => q.id).join(',')),
    [questions],
  );
  ...
  // Auto-save enquanto a prova não terminou
  useEffect(() => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{wasAnswered ? (AREA_LABELS[q.area] || q.area) : `Questao ${i + 1}`}</p>
                    <p className="text-xs text-muted-foreground">
                      {wasAnswered ? `Resposta: ${userAnswer} | Gabarito: ${q.gabarito}` : 'Nao respondida'}
                    </p>
                  </div>
          ...
          {restoredBanner && (
            <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3">
              <div className="flex items-center gap-2 text-sm text-primary">
                ...
                <span>
                  Progresso restaurado — questão {currentIndex + 1}
                  {Object.keys(answers).length > 0 && ` (${Object.keys(answers).length} respondida${Object.keys(answers).length > 1 ? 's' : ''})`}.
                </span>
              </div>
              <button onClick={() => setRestoredBanner(false)} className="text-xs font-semibold text-primary hover:underline shrink-0">OK</button>
            </div>
          )}
          {/* Area badge — only show after answering to not give away the answer */}
            ...
            <div className="pt-2">
              {!isRevealed ? (
                ...
                <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 space-y-3">
                  ...
                  {currentQ.explicacao ? (
                    <p className="text-sm text-muted-foreground leading-relaxed">{currentQ.explicacao}</p>
                  ) : (
                    <div className="space-y-2 border-t border-primary/10 pt-3">
                      <p className="text-sm text-muted-foreground">
                        A alternativa correta é <strong className="text-primary">{currentQ.gabarito}</strong>.
                        {userAnswer === currentQ.gabarito
                          ? ' Você acertou!'
                      ...
                      <div className="space-y-1.5">
                        {alternatives.map((alt) => {
                          const isCorrectAlt = alt.letter === currentQ.gabarito;
                          const isUserChoice = alt.letter === userAnswer;
                          return (
                            <div
                              key={alt.letter}
                            ...
                            >
                              <span className="font-bold shrink-0">{alt.letter})</span>
                              <span className="flex-1">{alt.text}</span>
                              {isCorrectAlt && <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5" />}
                              {isUserChoice && !isCorrectAlt && <XCircle className="h-4 w-4 shrink-0 mt-0.5" />}
                            </div>
                          );
                        })}
                      </div>
                      <p className="text-xs text-muted-foreground/70 italic pt-1">
                        💬 Use o chat ao lado para pedir uma explicação detalhada sobre esta questão.
                      </p>
                    </div>
                  )}
                </div>
