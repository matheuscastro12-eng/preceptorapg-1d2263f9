...
const SimulationView = ({ resultado, onExit, isGenerating = false, isComplete = true }: SimulationViewProps) => {
  ...
  if (questions.length === 0 && !isGenerating) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-16 text-center">
        ...
        <Button variant="outline" onClick={onExit} className="mt-4">Voltar para visualização completa</Button>
      </div>
    );
  }
  ...
  // Results screen — use PostSimulationFeedback
  if (showResults) {
    return (
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">A próxima questão ainda está sendo gerada.</p>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mb-3 sm:mb-4 shrink-0">
        <div className="flex items-center justify-between mb-1.5 sm:mb-2">
          ...
          <span className="text-[11px] sm:text-xs text-muted-foreground">
            ...
            {isGenerating && <span className="ml-1.5 sm:ml-2 text-primary">• Gerando...</span>}
          </span>
        </div>
        ...
        <div className="space-y-4 pr-2">
          {(currentQ.type || currentQ.tema) && (
            <div className="flex items-center gap-2 flex-wrap">
              {currentQ.type && <span className="text-xs px-2 py-0.5 rounded-full bg-accent/20 text-accent font-medium">{currentQ.type}</span>}
              {currentQ.tema && <span className="text-xs px-2 py-0.5 rounded-full bg-primary/20 text-primary font-medium">{currentQ.tema}</span>}
            </div>
          )}
              ...
              {!isRevealed ? (
                ...
                <>
                  ...
                  {currentIndex < totalQuestions - 1 && (
                    <Button onClick={() => setCurrentIndex(currentIndex + 1)} className="w-full gap-2 bg-gradient-to-r from-primary to-primary/80" size="sm">
                      Próxima Questão <ChevronRight className="h-4 w-4" />
                    </Button>
                  )}
                  {currentIndex === totalQuestions - 1 && isGenerating && (
                    <Button onClick={() => setCurrentIndex(currentIndex + 1)} variant="outline" className="w-full gap-2" size="sm">
      ...
      <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-border/30 shrink-0 gap-2">
        <Button variant="outline" size="sm" onClick={() => setCurrentIndex(Math.max(0, currentIndex - 1))} disabled={currentIndex === 0} className="gap-1 h-9 sm:h-10 px-3 sm:px-4 text-xs sm:text-sm">
        ...
        {showFinishButton ? (
          <Button size="sm" onClick={finishExam} className="gap-1 bg-gradient-to-r from-primary to-primary/80 h-9 sm:h-10 px-3 sm:px-4 text-xs sm:text-sm">
            <Trophy className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Finalizar ({answeredCount}/{totalQuestions})
          </Button>
        ) : (
          <Button variant="outline" size="sm" onClick={() => setCurrentIndex(currentIndex + 1)} disabled={isLastAvailable && !isGenerating} className="gap-1">
